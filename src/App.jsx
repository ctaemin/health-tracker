...
export default function App() {
  ...
  const addExercise = () => {
    ...
    const entry = { id: Date.now(), type: exType, date: exDate, duration: parseInt(exDuration), memo: exMemo };
    setExercises(prev => [entry, ...prev]);
    setExDuration(""); setExMemo("");
  };

  const handleInbodyPhoto = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    setInbodyLoading(true);
    setInbodyResult(null);
    setInbodyError("");
    try {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: `이 InBody 결과지에서 다음 항목들을 추출해서 JSON 형식으로만 응답해주세요 (다른 텍스트 없이 JSON만):
{
  "date": "측정일(YYYY-MM-DD)",
  "weight": 체중(숫자),
  "muscle": 골격근량(숫자),
  ...
  "mineral": 무기질(숫자),
  "bmr": 기초대사량(숫자),
  "visceralFat": 내장지방레벨(숫자)
}
값을 찾을 수 없으면 null로 표시하세요.` }
            ]
          }]
        })
      });
      ...
      // Also add to weight records
      if (parsed.weight && parsed.date) {
        const entry = { date: parsed.date, weight: parsed.weight, bmi: parsed.bmi || 0, fat: parsed.fatPercent || 0 };
        setWeights(prev => [...prev.filter(w => w.date !== parsed.date), entry].sort((a,b) => a.date.localeCompare(b.date)));
      }
      if (parsed.date) {
        setInbodyRecords(prev => [{ ...parsed, id: Date.now() }, ...prev]);
      }
    } catch (err) {
      setInbodyError("사진 분석 중 오류가 발생했어요. 다시 시도해주세요.");
    }
    setInbodyLoading(false);
  };

  const chartData = weights.map(w => ({ date: fmtDate(w.date), weight: w.weight, bmi: w.bmi, fat: w.fat }));
    ...
    <>
      ...
      <div className="app">
        <div className="header">
          <div className="header-title">HealthLog</div>
          <div className="header-date">{new Date().toLocaleDateString("ko-KR", { month: "long", day: "numeric" })}</div>
        </div>
        <div className="tabs">
          {TABS.map((t, i) => (
        ...
        <div className="content">
          {tab === 0 && <Dashboard latestWeight={latestWeight} weightDelta={weightDelta} chartData={chartData} thisWeekEx={thisWeekEx} totalMinutes={totalMinutes} inbodyRecords={inbodyRecords} />}
          {tab === 1 && <WeightTab weights={weights} wDate={wDate} setWDate={setWDate} wWeight={wWeight} setWWeight={setWWeight} wBmi={wBmi} setWBmi={setWBmi} wFat={wFat} setWFat={setWFat} addWeight={addWeight} chartData={chartData} />}
          {tab === 2 && <InbodyTab fileRef={fileRef} handleInbodyPhoto={handleInbodyPhoto} inbodyLoading={inbodyLoading} inbodyResult={inbodyResult} inbodyError={inbodyError} inbodyRecords={inbodyRecords} />}
          {tab === 3 && <ExerciseTab exercises={exercises} exType={exType} setExType={setExType} exDate={exDate} setExDate={setExDate} exDuration={exDuration} setExDuration={setExDuration} exMemo={exMemo} setExMemo={setExMemo} addExercise={addExercise} />}
        </div>
      </div>
    </>
  );
...
function Dashboard({ latestWeight, weightDelta, chartData, thisWeekEx, totalMinutes, inbodyRecords }) {
  return (
    <div>
      <div className="card">
        <div className="card-title">오늘 현황</div>
        <div className="stat-grid">
          <div className="stat-item">
            <div className="stat-value">{latestWeight?.weight ?? "--"}</div>
            <div className="stat-label">체중 kg</div>
            {weightDelta !== null && <div className={`stat-delta ${parseFloat(weightDelta) <= 0 ? "delta-down" : "delta-up"}`}>{parseFloat(weightDelta) > 0 ? "+" : ""}{weightDelta}</div>}
          </div>
          <div className="stat-item">
            <div className="stat-value">{latestWeight?.bmi ?? "--"}</div>
            <div className="stat-label">BMI</div>
          </div>
          <div className="stat-item">
            <div className="stat-value">{latestWeight?.fat ?? "--"}</div>
            <div className="stat-label">체지방 %</div>
          </div>
        </div>
      </div>

      <div className="card">
        <div className="card-title">체중 추이</div>
        <ResponsiveContainer width="100%" height={140}>
          <AreaChart data={chartData}>
            ...
            <YAxis domain={["auto", "auto"]} tick={{ fill: "#64748b", fontSize: 10 }} axisLine={false} width={30} />
            <Tooltip contentStyle={{ background: "#111827", border: "1px solid #1e2d45", borderRadius: 8, color: "#e8edf5", fontSize: 12 }} />
            <Area type="monotone" dataKey="weight" stroke="#3b82f6" strokeWidth={2} fill="url(#wg)" dot={{ fill: "#3b82f6", r: 3 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>

      <div className="card">
        <div className="card-title">이번 주 운동</div>
        <div className="week-summary">
          <div className="summary-box">
            <div className="summary-num">{thisWeekEx.length}</div>
            <div className="summary-desc">운동 횟수</div>
          </div>
          <div className="summary-box">
            <div className="summary-num">{totalMinutes}</div>
            <div className="summary-desc">총 운동 분</div>
          </div>
        </div>
        {thisWeekEx.length > 0 && (
          <div style={{ marginTop: 12 }}>
            {thisWeekEx.slice(0,3).map(ex => (
              <div key={ex.id} className="history-item">
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{ex.type}</div>
                  <div className="history-date">{fmtDate(ex.date)}</div>
                </div>
                <div style={{ color: "#3b82f6", fontFamily: "DM Mono", fontWeight: 700 }}>{ex.duration}분</div>
              </div>
            ))}
          </div>
        )}
      ...
      {inbodyRecords.length > 0 && (
        <div className="card">
          <div className="card-title">최근 인바디</div>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div className="score-circle" style={{ "--pct": inbodyRecords[0].score }}>
              <div className="score-inner">
                <div className="score-num">{inbodyRecords[0].score}</div>
                <div className="score-label">점수</div>
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <div className="inbody-row"><span className="inbody-key">측정일</span><span className="inbody-val">{inbodyRecords[0].date}</span></div>
              <div className="inbody-row"><span className="inbody-key">골격근량</span><span className="inbody-val">{inbodyRecords[0].muscle}kg</span></div>
              <div className="inbody-row"><span className="inbody-key">체지방률</span><span className="inbody-val">{inbodyRecords[0].fatPercent}%</span></div>
            </div>
          </div>
        </div>
      )}
...
function WeightTab({ weights, wDate, setWDate, wWeight, setWWeight, wBmi, setWBmi, wFat, setWFat, addWeight, chartData }) {
  return (
    <div>
      <div className="card">
        <div className="card-title">체중 입력</div>
        <div className="input-group">
          <label className="input-label">날짜</label>
          <input className="input-field" type="date" value={wDate} onChange={e => setWDate(e.target.value)} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 14 }}>
          <div>
            <label className="input-label">체중 (kg)</label>
            <input className="input-field" type="number" step="0.1" placeholder="75.0" value={wWeight} onChange={e => setWWeight(e.target.value)} />
          </div>
          <div>
            <label className="input-label">BMI</label>
            <input className="input-field" type="number" step="0.1" placeholder="27.0" value={wBmi} onChange={e => setWBmi(e.target.value)} />
          </div>
          <div>
            <label className="input-label">체지방%</label>
            <input className="input-field" type="number" step="0.1" placeholder="28.0" value={wFat} onChange={e => setWFat(e.target.value)} />
          </div>
        </div>
        <button className="btn btn-primary" onClick={addWeight}>기록 저장</button>
      </div>

      <div className="card">
        <div className="card-title">체지방률 추이</div>
        <ResponsiveContainer width="100%" height={130}>
          <AreaChart data={chartData}>
            ...
            <YAxis domain={["auto", "auto"]} tick={{ fill: "#64748b", fontSize: 10 }} axisLine={false} width={30} />
            <Tooltip contentStyle={{ background: "#111827", border: "1px solid #1e2d45", borderRadius: 8, color: "#e8edf5", fontSize: 12 }} />
            <Area type="monotone" dataKey="fat" stroke="#06b6d4" strokeWidth={2} fill="url(#fg)" dot={{ fill: "#06b6d4", r: 3 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>

      <div className="card">
        <div className="card-title">기록 히스토리</div>
        {[...weights].reverse().map((w, i) => (
          <div key={i} className="history-item">
            <div>
              <div className="history-date">{w.date}</div>
              <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>BMI {w.bmi} · 체지방 {w.fat}%</div>
            </div>
            <div className="history-val" style={{ color: "#e8edf5" }}>{w.weight}<span style={{ fontSize: 11, color: "#64748b" }}>kg</span></div>
          </div>
        ))}
      </div>
    </div>
